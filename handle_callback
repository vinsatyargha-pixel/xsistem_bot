...
import time
import requests  # HANYA INI YANG DIPERLUKAN
...
def parse_report_text(text):
    ...
    data = {'original_text': text}  # SIMPAN TEKS ASLI UNTUK APPS SCRIPT
    lines = text.split('\n')
...
def handle_report_message(message):
    ...
    try:
        ...
        # Parse data (DENGAN original_text)
        ...
        if missing_fields:
            bot.reply_to(message, f"❌ Data kurang: {', '.join(missing_fields)}")
            return
        ...
        if success:
            response = (
                f"✅ *REPORT BERHASIL DISIMPAN!*\n"
                f"👤 User ID: {data.get('user_id')}\n"
                f"🏦 Bank Member: {data.get('bank_member')}\n"
...
# ========== RESET PASSWORD HANDLERS (EXISTING) ==========
...
if __name__ == "__main__":
    ...
    print("📋 Report Crossbank aktif: /report")
